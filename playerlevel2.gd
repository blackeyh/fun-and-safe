...
func _physics_process(delta):
	...
	if can_move:
		# Movement input
		if Input.is_action_pressed("move_left") and not is_bouncing :
			if  is_on_floor():
				sprite.play("walk")
			velocity.x = -speed
			  # Play walk animation when moving left
		elif Input.is_action_pressed("move_right") and not is_bouncing :
			velocity.x = speed
			if  is_on_floor():
				sprite.play("walk")
		elif is_on_floor() and (sprite.animation == "hurt" or sprite.animation == "jump"):
				velocity.x = 0
